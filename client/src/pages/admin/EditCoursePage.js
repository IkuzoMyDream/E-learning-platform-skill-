...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setName(course.attributes.name)
        setPrice(course.attributes.price)
        setDescription(course.attributes.description)
        setNameTeacher(course.attributes.name_teacher)
        setEmailTeacher(course.attributes.mail_teacher)
        
        

        setCourseData(
          { ...course.attributes,
           categories: course.attributes.categories.data.map(item => {return{...item.attributes,id:item.id}})}
          //   {
          //   name: "test",
  ...
  useEffect(() => {
    setSelectedCategoriesId(courseData.categories.map(item=>item.id))
